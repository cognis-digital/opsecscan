"""Native cognis-connect emit for opsecscan — forward findings to any platform.

Maps opsecscan's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    opsecscan ... --format json | opsecscan-emit --to stix
    opsecscan-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('key', 'Unknown')
        out['severity'] = rec.get('level', 'info').lower()
        out['type'] = rec.get('category', 'unknown')
