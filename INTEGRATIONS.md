...
**opsecscan** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
the suite's integration SDK. It maps any tool's JSON into a canonical **Finding** and
forwards it to the platforms that fit the **Geospatial / GEOINT** domain.
...
Once `opsecscan` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
opsecscan ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
opsecscan ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
opsecscan ... --format json | cognis-connect emit --to brief   # analyst brief via your /v1 fleet
```
...
works whatever `opsecscan` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="opsecscan") for rec in records]   # records = your JSON output
